...
const AllCompanyJob = () => {
  ...
  const getCompanyJob = async()=>{
      let res = await axios.get(`https://jobbackend-5yqy.onrender.com/job/singlecompanyalljobs`,{
        headers:{
  ...
  const handleDelete = async(id)=>{
    console.log(id)
    let res = await axios.delete(`https://jobbackend-5yqy.onrender.com/job/delete/${id}`,{
      headers:{
        'Authorization':userStore.token
      }
    })
 ...
 const handleFileChanger = async(e)=>{
     ...
     const handleSubmit = async(e)=>{
       e.preventDefault()
       console.log(selectedJob)
       let res = await axios.put(`https://jobbackend-5yqy.onrender.com/job/update/${selectedJob._id}`,selectedJob,{
         headers:{
           'Authorization':userStore.token
         }
       })
  ...
  <div>
    ...
    {ele.requirement.length >0 &&<div className="flex flex-wrap items-center mt-2 text-gray-700 dark:text-gray-200">
      <span className='shrink-0'>Requirements:-</span>
          {ele.requirement.map((requirements)=>{
            return <p className='ms-2'>{requirements},</p>
          })}
    </div>}
    <div className="flex items-center justify-end mt-4">
      ...
      <a onClick={()=>handleDelete(ele)} className="mr-2 text-gray-800 cursor-pointer dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300" tabIndex={0} role="link" aria-label="share link">
        ...
        <Modal cancelButtonProps={{style: { display: 'none' } }} okButtonProps={{style: { display: 'none' } }} className='' title="Basic Modal" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
